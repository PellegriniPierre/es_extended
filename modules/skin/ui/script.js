$(document).ready(function(){
  ...
  $('input').change(function(){
    $.post('http://esx_skin/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    if(e.which == 97){ // A pressed
      $.post('http://esx_skin/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('http://esx_skin/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#tabs label').on('click', function(e){
    //e.preventDefault();
    $.post('http://esx_skin/zoom', JSON.stringify({
